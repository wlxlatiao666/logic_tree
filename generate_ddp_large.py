...
def run_inference_tpdp(args):
    ...
    # 保存最终结果路径
    if test_size == -1:
        output_path = f"./results/{model_name}/{dataset}/logic_tree_results_all_leaves{num_leaves}_threshold{tau}_tpdp.json"
    else:
        output_path = f"./results/{model_name}/{dataset}/logic_tree_results_{test_size}_leaves{num_leaves}_threshold{tau}_tpdp.json"
